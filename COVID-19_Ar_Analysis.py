...
#regions = ["BUENOS AIRES", "CABA"]
regions = ["CHACO", "CORDOBA", "JUJUY", "MENDOZA", "RIO NEGRO", "SANTA FE", "ENTRE RIOS", "SALTA", "LA RIOJA"]
...
newConfirmedCasesTrend7 = False #Decide if you want to plot new daily cases trend (5 day average)
...
newDeathsTrend7 = False #Decide if you want to plot new daily deaths trend (7 day average)
...
def plotDoublebyOutbreak(regions, datatypeA, datatypeB, dataguide, xtitle, ytitleA, ytitleB, ticksIntervalA, ticksIntervalB, savechart, show):
	...
	plt.ylabel("")
	plt.xlabel(xtitle, fontname=legendFont)
	plt.legend(loc=0, shadow = True, facecolor = backgroundFigure, prop={'family' : legendFont, 'size' : 7})
	#Setting up grid...
	gridAndTicks(s[1]*1.1, ticksIntervalB)
	plt.tight_layout(rect=[0, 0, 1, 1])
	if savechart == True:
